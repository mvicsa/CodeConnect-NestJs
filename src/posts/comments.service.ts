...
export class CommentsService {
  ...
  async create(data: Omit<Comment, 'createdBy'>, userId: string): Promise<Comment> {
  ...
  async findByPost(postId: string): Promise<Comment[]> {
    return this.commentModel.find({ postId, parentCommentId: null })
  ...
  async findReplies(parentCommentId: string): Promise<Comment[]> {
    return this.commentModel.find({ parentCommentId })
      .populate('createdBy', '-password')
      .populate({
        path: 'userReactions.userId',
        select: '_id firstName lastName avatar role',
  ...
  async update(_id: string, data: Partial<Comment>, userId: string): Promise<Comment> {
    const comment = await this.commentModel.findById(_id);
    if (!comment) throw new NotFoundException('Comment not found');
    if (comment.createdBy.toString() !== userId) throw new ForbiddenException('You can only edit your own comments');
  ...
  async delete(_id: string, userId: string): Promise<void> {
    const comment = await this.commentModel.findById(_id);
    if (!comment) throw new NotFoundException('Comment not found');
    if (comment.createdBy.toString() !== userId) throw new ForbiddenException('You can only delete your own comments');
  ...
  async addOrUpdateReaction(commentId: string, userId: string, username: string, reaction: string): Promise<Comment> {
    const comment = await this.commentModel.findById(commentId);
    if (!comment) throw new NotFoundException('Comment not found');
    
    // Check if user already has the same reaction
    const existingReaction = comment.userReactions.find(ur => ur.userId.toString() === userId);
    
    if (existingReaction && existingReaction.reaction === reaction) {
      // Remove the reaction if it's the same
      comment.userReactions = comment.userReactions.filter(ur => ur.userId.toString() !== userId);
    } else {
    ...
    comment.reactions = reactionTypes.reduce((acc, type) => {
      acc[type] = comment.userReactions.filter(ur => ur.reaction === type).length;
      return acc;
    }, {} as any);
