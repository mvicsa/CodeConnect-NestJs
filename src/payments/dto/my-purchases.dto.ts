...
export class MyPurchasesQueryDto {
  ...
  @ApiProperty({ required: false, description: 'Start date for filtering purchases (ISO 8601 string)' })
  @IsOptional()
  @IsDateString()
  startDate?: string;

  @ApiProperty({ required: false, description: 'End date for filtering purchases (ISO 8601 string)' })
  @IsOptional()
  @IsDateString()
  endDate?: string;

  @ApiProperty({ required: false, description: 'Currency to filter purchases by' })
  @IsOptional()
  @IsString()
  currency?: string;

  @ApiProperty({ required: false, description: 'Room ID to filter purchases by' })
  @IsOptional()
  @IsString()
  roomId?: string;
...
export class UserPurchaseDto {
  @ApiProperty({ description: 'ID of the purchase record' })
  _id: string;
  ...
  @ApiProperty({ description: 'Status of the payment', enum: ['pending', 'completed', 'failed', 'refunded', 'cancelled', 'expired'] })
  status: string;
...
export class MyPurchasesResponseDto {
  @ApiProperty({ type: [UserPurchaseDto], description: 'List of user\'s purchase records' })
  purchases: UserPurchaseDto[];

  @ApiProperty({ description: 'Total amount spent in USD', type: Number })
  totalSpentUSD: number;

  @ApiProperty({ description: 'Total number of purchase records' })
  totalCount: number;
