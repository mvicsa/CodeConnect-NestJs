...
export class NotificationService {
  ...
  async create(dto: CreateNotificationDto) {
    ...
    if (
      dto.type === NotificationType.COMMENT_REACTION &&
      dto.data &&
      dto.data._id
    ) {
      ...
      if (existing) {
        ...
        if (populatedNotification) {
          ...
          if (this.gateway.server) {
            this.gateway.server
              .to(`user:${dto?.toUserId}`)
              .emit('notification:update', populatedNotification);
          }
    ...
    if (
      dto.type === NotificationType.POST_REACTION &&
      dto.data &&
      dto.data.postId
    ) {
      const existing = await this.notificationModel.findOne({
    ...
    if (populatedNotification) {
      ...
      if (
        ...
        typeof notification.data.postId === 'object'
      ) {
      ...
      if (
        ...
        typeof notification.data.commentId === 'object'
      ) {
  ...
  async addNotifications(notifications: CreateNotificationDto[]) {
    try {
      ...
      const populatedNotifications = await this.notificationModel
        .find({
